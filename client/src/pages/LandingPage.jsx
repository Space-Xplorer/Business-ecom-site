  ...
  return (
    <>
      ...
      <main className="min-h-[calc(100vh-200px)] bg-gradient-to-br from-[#FFF8E1] via-[#FFD700]/20 to-[#F26A1B]/10 relative overflow-x-hidden">
        ...
        <section className="w-full bg-black flex flex-col items-center justify-center py-4 border-b-4 border-[#F26A1B] relative z-10 shadow-2xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[1,2,3,4].map(i => (
              <div key={i} className="bg-white rounded-xl shadow-lg overflow-hidden border-2 border-[#FFD700]/40 hover:scale-105 hover:shadow-2xl transition-transform duration-300 group relative">
                <img src={`https://source.unsplash.com/300x300/?saree,assam,clothing&sig=${i}`} alt="New Arrival" className="w-full h-48 object-cover group-hover:opacity-90" />
                <div className="p-3 text-center">
                  <span className="font-semibold text-gray-800 text-lg group-hover:text-[#F26A1B] transition">Assamese Saree #{i}</span>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[1,2,3].map(i => (
              <div key={i} className="relative rounded-xl overflow-hidden shadow-lg border-2 border-[#FFD700]/40 hover:scale-105 hover:shadow-2xl transition-transform duration-300 group">
                <img src={`https://source.unsplash.com/400x250/?textile,assam,offer&sig=${i}`} alt="Deal" className="w-full h-56 object-cover group-hover:opacity-90" />
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4 z-10">
                  <span className="text-lg font-bold text-[#FFD700] drop-shadow">Special Deal #{i}</span>
